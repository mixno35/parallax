function setParallax(element, configs) {
  ...
  let conf = {
    ...
    outIn: configs != undefined && configs.outIn ? Boolean(configs.outIn) : false,
  }
  ...
  for (const item of parallax) {
    ...
    item.addEventListener('mousemove', e => {
      ...
      if (conf.outIn == true) {
        xAxis = -(xPercent * (conf.maxTilt / 100)).toFixed(2);
        yAxis = -(yPercent * (conf.maxTilt / 100)).toFixed(2);
      }
